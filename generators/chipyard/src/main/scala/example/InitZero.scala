...
class InitZero(implicit p: Parameters) extends LazyModule {
  val node = TLClientNode(
    Seq(TLMasterPortParameters.v1(Seq(TLClientParameters(name = "init-zero", sourceId = IdRange(0, 1)))))
  )
...
class InitZeroModuleImp(outer: InitZero) extends LazyModuleImp(outer) {
  ...
  mem.a.bits  := edge.Put(fromSource = 0.U, toAddress = addr, lgSize = log2Ceil(blockBytes).U, data = 0.U)._2
...
class WithInitZero(base: BigInt, size: BigInt)
    extends Config((site, here, up) => { case InitZeroKey =>
      Some(InitZeroConfig(base, size))
    })
