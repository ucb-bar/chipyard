...
case class PowerDomain(
  val name:        String,
  val modules:     ListBuffer[LazyModule],
  val isTop:       Boolean,
  val isGated:     Boolean,
  ...
  val mainVoltage = isGated match {
    case true  =>
      highVoltage // gated nets should have access to high voltage rail (since they are being gated to optimize power)
...
object UPFGenerator {
  ...
  def createPowerSwitches(pd: PowerDomain): String = {
    ...
    var subMessage = pd.isGated match {
      case true  =>
        s"""create_power_switch sw_${pd.name} -domain ${pd.name} -input_supply_port "psw_VDDH VDDH" """ +
