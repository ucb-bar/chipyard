...
class PeripheralAXI4Lite(params: PeripheralDeviceParams, beatBytes: Int)(implicit p: Parameters) extends ClockSinkDomain(ClockSinkParameters())(p) {
...
trait CanHaveAXI4Lite { this: BaseSubsystem =>
  private val portName = "axi4_lite"
  private val pbus = locateTLBusWrapper(PBUS)

  val axi4_lite = p(PeripheralDeviceKey) match {
    case Some(params) => {
      val peripheral = LazyModule(new PeripheralAXI4Lite(params, 4)(p))
      ...
      val axi4_lite_top = InModuleBody {
        val axi4_lite_io = IO(new AXI4Lite()).suggestName("axi4_lite")
        axi4_lite_io <> peripheral.module.io
        axi4_lite_io
      }
      Some(axi4_lite_top)
    }
