...
import chipyard._
import chipyard.clocking.{ClockWithFreq, HasChipyardPRCI}
...
object ApplyHarnessBinders {
  def apply(th: HasHarnessInstantiators, sys: LazyModule, portMap: Map[String, Seq[Data]])(implicit p: Parameters)
    : Unit = {
...
class HarnessBinder[T, S <: HasHarnessInstantiators, U <: Data](
  composer:           ((T, S, Seq[U]) => Unit) => (T, S, Seq[U]) => Unit
)(implicit systemTag: ClassTag[T],
  harnessTag:         ClassTag[S],
  portTag:            ClassTag[U],
) extends Config((site, here, up) => { case HarnessBinders =>
      up(HarnessBinders, site) + (systemTag.runtimeClass.toString ->
...
class OverrideHarnessBinder[T, S <: HasHarnessInstantiators, U <: Data](
  fn:           => (T, S, Seq[U]) => Unit
...
class ComposeHarnessBinder[T, S <: HasHarnessInstantiators, U <: Data](
  fn:           => (T, S, Seq[U]) => Unit
)(implicit tag: ClassTag[T],
  thtag:        ClassTag[S],
  ptag:         ClassTag[U],
) extends HarnessBinder[T, S, U]((upfn: (T, S, Seq[U]) => Unit) =>
      (t, th, p) => {
        upfn(t, th, p)
        fn(t, th, p)
      }
...
class WithSimNetwork
    extends OverrideHarnessBinder({
      (system: CanHavePeripheryIceNIC,
        th:    BaseModule with HasHarnessInstantiators,
        ports: Seq[ClockedIO[NICIOvonly]],
      ) =>
        {
          implicit val p: Parameters = GetSystemParameters(system)
...
class WithSimAXIMemOverSerialTL
    extends OverrideHarnessBinder({
...
// DOC include start: HarnessClockInstantiatorEx
                      ...
                      val success                 = SimTSI.connect(
                        Some(harnessMultiClockAXIRAM.module.io.tsi),
                      ...
                      (harnessMultiClockAXIRAM.mem_axi4.get.zip(harnessMultiClockAXIRAM.memNode.get.edges.in)).map {
                        case (axi_port, edge) =>
                          val memSize  = serialTLManagerParams.memParams.size
                          val memBase  = serialTLManagerParams.memParams.base
                          val lineSize = p(CacheBlockBytes)
                          val mem      = Module(new SimDRAM(memSize, lineSize, BigInt(memFreq.toLong), memBase, edge.bundle))
                            .suggestName("simdram")
        ...
        (system: CanHaveMasterAXI4MemPort, th: HasHarnessInstantiators, ports: Seq[ClockedAndResetIO[AXI4Bundle]]) =>
          {
            val p: Parameters = chipyard.iobinders.GetSystemParameters(system)
            (ports
              .zip(system.memAXI4Node.edges.in))
              .map {
                case (port, edge) =>
                  ...
                  val mem       =
                    Module(new SimDRAM(memSize, lineSize, clockFreq, memBase, edge.bundle)).suggestName("simdram")
                  ...
                  if (additionalLatency > 0) {
                    withClockAndReset(port.clock, port.reset) {
                      mem.io.axi.aw <> (0 until additionalLatency)
                        .foldLeft(Decoupled(port.bits.aw))((t, _) => Queue(t, 1, pipe = true))
                      mem.io.axi.w  <> (0 until additionalLatency)
                        .foldLeft(Decoupled(port.bits.w))((t, _) => Queue(t, 1, pipe = true))
                      port.bits.b   <> (0 until additionalLatency)
                        .foldLeft(Decoupled(mem.io.axi.b))((t, _) => Queue(t, 1, pipe = true))
                      mem.io.axi.ar <> (0 until additionalLatency)
                        .foldLeft(Decoupled(port.bits.ar))((t, _) => Queue(t, 1, pipe = true))
                      port.bits.r   <> (0 until additionalLatency)
                        .foldLeft(Decoupled(mem.io.axi.r))((t, _) => Queue(t, 1, pipe = true))
                    }
                  }
...
class WithSimDebug
    extends OverrideHarnessBinder({ (system: HasPeripheryDebug, th: HasHarnessInstantiators, ports: Seq[Data]) =>
      {
        implicit val p: Parameters = GetSystemParameters(system)
        ports.map {
          case d: ClockedDMIIO =>
            ...
            val dtm         =
              Module(new TestchipSimDTM).connect(th.harnessBinderClock, th.harnessBinderReset.asBool, d, dtm_success)
          case j: JTAGChipIO   =>
            val dtm_success = WireInit(false.B)
            when(dtm_success) { th.success := true.B }
        ...
        (system: CanHavePeripheryUARTTSI, th: HasHarnessInstantiators, ports: Seq[UARTTSIIO]) =>
          {
            ...
            ports.map({ port =>
              ...
              val success              =
                SimTSI.connect(Some(TSIIO(serial_width_adapter.io.wide)), th.harnessBinderClock, th.harnessBinderReset)
              when(success) { th.success := true.B }
...
class WithTraceGenSuccess
    extends OverrideHarnessBinder({ (system: TraceGenSystemModuleImp, th: HasHarnessInstantiators, ports: Seq[Bool]) =>
      {
        ports.map { p => when(p) { th.success := true.B } }
      }
    })

class WithSimDromajoBridge
    extends ComposeHarnessBinder({
        ...
        (system: CanHaveTraceIOModuleImp, th: HasHarnessInstantiators, ports: Seq[TraceOutputTop]) =>
          {
            ...
            val cfg            = SpikeCosimConfig(
              ...
              priv       = tiles.headOption
                .map(t => if (t.usingUser) "MSU" else if (t.usingSupervisor) "MS" else "M")
                .getOrElse(""),
              ...
              has_dtm    =
                p(ExportDebug).protocols.contains(DMI), // assume that exposing clockeddmi means we will connect SimDTM
            )
...
class WithClockAndResetFromHarness
    extends OverrideHarnessBinder({ (system: HasChipyardPRCI, th: HasHarnessInstantiators, ports: Seq[Data]) =>
      {
