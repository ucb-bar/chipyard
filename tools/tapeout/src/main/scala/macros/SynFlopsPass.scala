...
class SynFlopsPass(synflops: Boolean, libs: Seq[Macro]) extends firrtl2.passes.Pass {
  ...
  lazy val libMods: Map[String, Module] = libs.map { lib =>
    lib.src.name -> {
      ...
      val maxDepth = firrtl2.Utils.min(lib.src.depth, 1 << 26)
