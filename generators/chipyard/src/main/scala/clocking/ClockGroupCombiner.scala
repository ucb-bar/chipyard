      ...
      groups.map { case (grouped_name, matched_names, unmatched_names) =>
        (
          ...
          (m: ClockSinkParameters) =>
            matched_names.exists(n => m.name.get.contains(n)) && !unmatched_names.exists(n => m.name.get.contains(n)),
        )
      }
    })

/** This node combines sets of clock groups according to functions provided in the ClockGroupCombinerKey The
  * ClockGroupCombinersKey contains a list of tuples of:
  *   - The name of the combined group
  *   - A function on the ClockSinkParameters, returning True if the associated clock group should be grouped by this
  *     node This node will fail if
...
class ClockGroupCombiner(implicit p: Parameters, v: ValName) extends LazyModule {
  ...
  val sinkFn: ClockGroupSinkParameters => ClockGroupSinkParameters       = { u =>
    var i               = 0
    val (grouped, rest) =
      combiners.map(_._2).foldLeft((Seq[ClockSinkParameters](), u.members)) { case ((grouped, rest), c) =>
        val (g, r) = rest.partition(c(_))
        val name   = combiners(i)._1
        i = i + 1
        require(g.size >= 1)
        val takes = g.map(_.take).flatten
        require(takes.distinct.size <= 1, s"Clock group $name has non-homogeneous requested ClockParameters $takes")
        require(takes.size > 0, s"Clock group $name has no inheritable frequencies")
