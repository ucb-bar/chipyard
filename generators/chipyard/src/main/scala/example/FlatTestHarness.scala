...
import freechips.rocketchip.prci.ClockSourceAtFreqFromPlusArg
import freechips.rocketchip.util.PlusArg
import freechips.rocketchip.subsystem.CacheBlockBytes
import freechips.rocketchip.devices.debug.SimJTAG
import freechips.rocketchip.jtag.JTAGIO
import testchipip.{SerialTLKey, SimDRAM, SimTSI, TSIHarness, UARTAdapter}
...
class FlatTestHarness(implicit val p: Parameters) extends Module {
  ...
  withClockAndReset(clock, reset) {
    ...
    val harnessMultiClockAXIRAM =
      TSIHarness.connectMultiClockAXIRAM(lazyDut.system.serdesser.get, serial_bits, clock, reset)
    ...
    (harnessMultiClockAXIRAM.mem_axi4.get.zip(harnessMultiClockAXIRAM.memNode.get.edges.in)).map {
      case (axi_port, edge) =>
        ...
        val mem      =
          Module(new SimDRAM(memSize, lineSize, BigInt(memFreq.toLong), memBase, edge.bundle)).suggestName("simdram")
