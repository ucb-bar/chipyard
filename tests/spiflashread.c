...
int main(void) {
  ...
  ffmt.fields.pad_cnt = 0;  // Our SPI flash model assumes 8 dummy cycles for
                            // fast reads, 0 for slow
  ffmt.fields.cmd_proto = SPIFLASH_PROTO_SINGLE; // Our SPI flash model only
                                                 // supports single-bit commands
  ffmt.fields.addr_proto =
      SPIFLASH_PROTO_SINGLE; // We support both single and quad
  ffmt.fields.data_proto =
      SPIFLASH_PROTO_SINGLE;   // We support both single and quad
  ...
  configure_spiflash(ffmt);
  if (test_spiflash(0x1000, 0x100, 0))
    return 1;
  ...
  configure_spiflash(ffmt);
  if (test_spiflash(0x2340, 0x100, 0))
    return 1;
  ...
  configure_spiflash(ffmt);
  if (test_spiflash(0x410c, 0x100, 0))
    return 1;
  ...
  configure_spiflash(ffmt);
  if (test_spiflash(0x5ff8, 0x100, 0))
    return 1;
  ...
  configure_spiflash(ffmt);
  if (test_spiflash(0x7c04, 0x100, 0))
    return 1;
  ...
  configure_spiflash(ffmt);
  if (test_spiflash(0x9000, 0x100, 0))
    return 1;

  printf("Testing SPI flash extended range...\n");
  // The provided memory image is only 1MiB, but the model has 16MiB of
  // addressable space This should return 0
  if (test_spiflash(0x100000, 0x100, 1))
    return 1;

  // This write should do nothing, so we can just re-test the first test
  printf("Testing that the SPI is not writable...\n");
  write_spiflash(test_data,
                 test_len,
                 0x0,
                 0x3E,
                 4,
