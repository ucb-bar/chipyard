...
/** This sort of node can be used when it is a connectivity passthrough, but modifies the flow of parameters (which may
  * result in changing the names of the underlying signals).
  */
...
/** Pushes the ClockGroup's name into each member's name field as a prefix. This is intended to be used before a
  * ClockGroupAggregator so that sources from different aggregated ClockGroups can be disambiguated by their names.
  */
...
/** [Word from on high is that Strings are in...] Overrides the take field of all clocks in a group, by attempting to
  * apply a series of assignment functions: (name: String) => freq-in-MHz: Option[Double] to each sink. Later functions
  * that return non-empty values take priority. The default if all functions return None.
  */
