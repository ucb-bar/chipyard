...
class LargeBoomAndRocketConfig
    extends Config(
...
class HwachaLargeBoomAndHwachaRocketConfig
    extends Config(
...
class LargeBoomAndHwachaRocketConfig
    extends Config(
...
class DualLargeBoomAndHwachaRocketConfig
    extends Config(
      new chipyard.config.WithMultiRoCC ++                     // support heterogeneous rocc
        new chipyard.config.WithMultiRoCCHwacha(0) ++          // put hwacha on hart-0 (rocket)
        new hwacha.DefaultHwachaConfig ++                      // set default hwacha config keys
...
class DualLargeBoomAndDualRocketConfig
    extends Config(
      new boom.common.WithNLargeBooms(2) ++                    // add 2 boom cores
...
class DualLargeBoomAndSingleRocketConfig
    extends Config(
      new boom.common.WithNLargeBooms(2) ++                    // add 2 boom cores
        new freechips.rocketchip.subsystem.WithNBigCores(1) ++ // add 1 rocket core
        new chipyard.config.WithSystemBusWidth(128) ++
...
class LargeBoomAndRocketWithControlCoreConfig
    extends Config(
