...
class DualRocketNoCConfig extends Config(
  new constellation.rc.WithConstellationNoCSystemBus(Seq(0, 0, 1), Seq(0, 2, 3, 4, 5)) ++
  ...
  new constellation.topology.WithMesh2DTopology(2, 3, RoutingRelation.mesh2DEscapeRouter) ++
  new freechips.rocketchip.subsystem.WithNBigCores(2) ++
  new freechips.rocketchip.subsystem.WithNBanks(4) ++
  new chipyard.config.AbstractConfig)

class QuadRocketNoCConfig extends Config(
  new constellation.rc.WithConstellationNoCSystemBus(Seq(0, 0, 1, 2, 3), Seq(0) ++ Seq.tabulate(16) { _ + 4 }) ++
  new constellation.routing.WithNBlockingVirtualNetworks(5) ++
  new constellation.channel.WithUniformNVirtualChannels(6, UserVirtualChannelParams(3)) ++
  new constellation.topology.WithMesh2DTopology(4, 5, RoutingRelation.mesh2DEscapeRouter) ++
...
class HexaRocketNoCConfig extends Config(
  new constellation.rc.WithConstellationNoCSystemBus(Seq(4, 0, 2, 5, 6, 9, 11), Seq(7, 1, 3, 8, 10)) ++
  new constellation.routing.WithNBlockingVirtualNetworks(5) ++
