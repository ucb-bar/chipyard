...
class MyTile(
  ...
  val memAXI4Node = AXI4MasterNode(
    Seq(
      AXI4MasterPortParameters(masters = Seq(AXI4MasterParameters(name = "myPortName", id = IdRange(0, 1 << idBits))))
    )
  )
...
class WithNMyCores(n: Int = 1, overrideIdOffset: Option[Int] = None)
