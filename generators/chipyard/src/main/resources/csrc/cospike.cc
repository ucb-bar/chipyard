...
public:
  read_override_device_t(std::string n, reg_t sz)
      : was_read_from(false), size(sz), name(n){};
...
extern "C" void cospike_set_sysinfo(char *isa,
                                    int vlen,
                                    char *priv,
                                    int pmpregions,
  ...
  if (!info) {
    ...
    // technically the targets aren't zicntr compliant, but they implement the
    // zicntr registers
...
extern "C" void cospike_cosim(long long int cycle,
  ...
  if (unlikely(!sim)) {
    ...
    std::string visa =
        "vlen:" + std::to_string(info->vlen ? info->vlen : 128) + ",elen:64";
    ...
    std::shared_ptr<rom_device_t> boot_rom =
        std::make_shared<rom_device_t>(info->bootrom);
    ...
    std::shared_ptr<read_override_device_t> clint =
        std::make_shared<read_override_device_t>("clint", CLINT_SIZE);
    std::shared_ptr<read_override_device_t> uart =
        std::make_shared<read_override_device_t>("uart", UART_SIZE);
    std::shared_ptr<read_override_device_t> plic =
        std::make_shared<read_override_device_t>("plic", PLIC_SIZE);
    ...
    bus_t temp_mem_bus;
    for (auto &pair : mems)
      temp_mem_bus.add_device(pair.first, pair.second);
    ...
    for (int i = 0; i < info->nharts; i++) {
      ...
      // HACKS: Our processor's don't implement zicntr fully, they don't provide
      // time
  ...
  if (dtm && dtm->loadarch_done && !spike_loadarch_done) {
    ...
    s->pc = ls.pc;
    s->prv = ls.prv;
    s->csrmap[CSR_MSTATUS]->write(s->csrmap[CSR_MSTATUS]->read() | MSTATUS_VS |
                                  MSTATUS_XS | MSTATUS_FS);
#define RESTORE(CSRID, csr) s->csrmap[CSRID]->write(ls.csr);
    ...
    if (ls.VLEN != p->VU.VLEN) {
      COSPIKE_PRINTF(
          "VLEN mismatch loadarch: $d != spike: $d\n", ls.VLEN, p->VU.VLEN);
      abort();
    }
    if (ls.ELEN != p->VU.ELEN) {
      COSPIKE_PRINTF(
          "ELEN mismatch loadarch: $d != spike: $d\n", ls.ELEN, p->VU.ELEN);
      abort();
    }
  ...
  if (valid || raise_interrupt || raise_exception) {
    p->clear_waiting_for_interrupt();
    for (auto &e : read_override_devices)
      e.get()->was_read_from = false;
    p->step(1);
  ...
  if (valid && !raise_exception) {
    if (s_pc != iaddr) {
      COSPIKE_PRINTF(
          "%d PC mismatch spike %llx != DUT %llx\n", cycle, s_pc, iaddr);
      if (unlikely(cospike_debug)) {
        COSPIKE_PRINTF("spike mstatus is %lx\n", s->mstatus->read());
    ...
    for (auto memwrite : mem_write) {
      ...
      if (waddr == tohost_addr && w_data >= info->mem0_base &&
          w_data < (info->mem0_base + info->mem0_size)) {
    ...
    for (auto &regwrite : log) {
      ...
      for (auto &e : read_override_devices)
        if (e.get()->was_read_from)
          device_read = true;

      bool lr_read = ((insn & MASK_LR_D) == MATCH_LR_D) ||
                     ((insn & MASK_LR_W) == MATCH_LR_W);
      bool sc_read = ((insn & MASK_SC_D) == MATCH_SC_D) ||
                     ((insn & MASK_SC_W) == MATCH_SC_W);

      bool ignore_read =
          sc_read ||
          (!mem_read.empty() &&
           (magic_addrs.count(mem_read_addr) || device_read || lr_read ||
      ...
      if ((rd != 0 && type == 0) || type == 1) {
        ...
        if (csr_read && ((csr_addr == 0x301) || // misa
                         ...
                         (csr_addr >= 0x7a0 &&
                          csr_addr <= 0x7aa) || // debug trigger registers
      ...
      //   COSPIKE_PRINTF("Scalar wdata behavior divergence between spike and
      //   DUT\n"); exit(-1);
      // }
    }
