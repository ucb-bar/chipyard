...
import freechips.rocketchip.devices.debug.{DMI, ExportDebug}
  ...
  val icacheNode = TLClientNode(
    Seq(
      TLMasterPortParameters.v1(
        Seq(TLMasterParameters.v1(sourceId = IdRange(0, 1), name = s"Core ${staticIdForMetadataUseOnly} ICache"))
      )
    )
  )
  ...
  val mmioNode = TLClientNode(
    (Seq(
      TLMasterPortParameters.v1(
        Seq(
          TLMasterParameters
            .v1(name = s"Core ${staticIdForMetadataUseOnly} MMIO", sourceId = IdRange(0, 1), requestFifo = true)
        )
      )
    ))
  )
  ...
  if (dcacheEdge.manager.anySupportAcquireB) {
    dcache_tl.a.bits := dcacheEdge
      .AcquireBlock(
        ...
        lgSize          = blockBits.U,
        growPermissions = Mux(spike.io.dcache.a.state_old, 2.U, Mux(spike.io.dcache.a.state_new, 1.U, 0.U)),
      )
      ._2
  } else {
...
class WithNSpikeCores(n: Int = 1, tileParams: SpikeTileParams = SpikeTileParams(), overrideIdOffset: Option[Int] = None)
