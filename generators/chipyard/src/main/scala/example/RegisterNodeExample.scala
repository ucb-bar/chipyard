...
class MyDeviceController(implicit p: Parameters) extends LazyModule {
  ...
  val node   =
    TLRegisterNode(address = Seq(AddressSet(0x10028000, 0xfff)), device = device, beatBytes = 8, concurrency = 1)
...
class MyAXI4DeviceController(implicit p: Parameters) extends LazyModule {
  val node = AXI4RegisterNode(address = AddressSet(0x10029000, 0xfff), beatBytes = 8, concurrency = 1)
...
class MyQueueRegisters(implicit p: Parameters) extends LazyModule {
  ...
  val node   =
    TLRegisterNode(address = Seq(AddressSet(0x1002a000, 0xfff)), device = device, beatBytes = 8, concurrency = 1)
...
// DOC include start: MyQueueRegisters
    ...
    node.regmap(0x00 -> Seq(RegField(64, queue.io.deq, queue.io.enq)))
...
class MySeparateQueueRegisters(implicit p: Parameters) extends LazyModule {
  ...
  val node   =
    TLRegisterNode(address = Seq(AddressSet(0x1002b000, 0xfff)), device = device, beatBytes = 8, concurrency = 1)

  lazy val module = new LazyModuleImp(this) {
    val queue = Module(new Queue(UInt(64.W), 4))

// DOC include start: MySeparateQueueRegisters
    node.regmap(0x00 -> Seq(RegField.r(64, queue.io.deq)), 0x08 -> Seq(RegField.w(64, queue.io.enq)))
...
class MyCounterRegisters(implicit p: Parameters) extends LazyModule {
  ...
  val node   =
    TLRegisterNode(address = Seq(AddressSet(0x1002c000, 0xfff)), device = device, beatBytes = 8, concurrency = 1)
...
// DOC include start: MyCounterRegisters
    ...
    node.regmap(0x00 -> Seq(RegField.r(64, readCounter(_))), 0x08 -> Seq(RegField.w(64, writeCounter(_, _))))
...
class MyCounterReqRespRegisters(implicit p: Parameters) extends LazyModule {
  ...
  val node   =
    TLRegisterNode(address = Seq(AddressSet(0x1002d000, 0xfff)), device = device, beatBytes = 8, concurrency = 1)
...
// DOC include start: MyCounterReqRespRegisters
    ...
    node.regmap(0x00 -> Seq(RegField.r(64, readCounter(_, _))), 0x08 -> Seq(RegField.w(64, writeCounter(_, _, _))))
