...
class ZCU106TSIHarness(override implicit val p: Parameters) extends ZCU106TSIShell with HasHarnessSignalReferences
{
  ...
  println(s"ZCU106TSI FPGA Base Clock Freq: ${dp(DefaultClockFrequencyKey)} MHz")
  ...
  val ddrOverlay = dp(DDROverlayKey).head.place(DDRDesignInput(dp(ExtTLMem).get.master.base, dutWrangler.node, harnessSysPLLNode)).asInstanceOf[DDRZCU106TSIPlacedOverlay]
  ...
  InModuleBody {
    ...
    val sys_clk_mhz = clockOverlay.overlayOutput.node.out.head._1.clock

    // Blink the status LEDs for sanity
    withClockAndReset(sys_clk_mhz, pllReset) {
    ...
    //other_leds(0) := ddrOverlay.mig.module.io.port.init_calib_complete
