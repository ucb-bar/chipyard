...
// point size (and therefore integer width/uint32_t) determined by IOWidth from
// Tail.scala point size is 2 * IOWidth since both real and imaginary components
// get IOWidth bits
...
int main(void) {
  ...
  for (int i = 0; i < num_points; i++) {
    volatile uint32_t *ptr_0 =
        (volatile uint32_t *)(FFT_RD_LANE_BASE + (i * 8));
    uint32_t read_val = *ptr_0;

    /* To convert binary to floating point
     * However, RISC-V compiler can't print floats. You can use this by
     * copy-pasting it into an online C compiler. The printf at the bottom of
     * this for loop will print out the values of real_part_bin and
     * imag_part_bin
     *
    ...
    if (read_val != expected_outputs[i]) {
      printf("FAIL: Expected and real differed: %x %x",
             read_val,
             expected_outputs[i]);
      return -1;
    }
