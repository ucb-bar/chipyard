...
/** Make DspBlock wrapper for StreamingPassthrough
  ...
  * @tparam B
  * @tparam T
  *   Type parameter for passthrough, i.e. FixedPoint or DspReal
  */
...
/** TLDspBlock specialization of StreamingPassthrough
  ...
  * @param p
  * @tparam T
  *   Type parameter for passthrough data type
  */
class TLStreamingPassthroughBlock[T <: Data: Ring](
  val proto:  T
)(implicit p: Parameters
) extends StreamingPassthroughBlock[TLClientPortParameters, TLManagerPortParameters, TLEdgeOut, TLEdgeIn, TLBundle, T](
      proto
    )
    with TLDspBlock
...
trait CanHavePeripheryStreamingPassthrough { this: BaseSubsystem =>
  val passthrough = p(StreamingPassthroughKey) match {
    case Some(params) => {
      ...
      pbus.coupleTo("streamingPassthrough") {
        streamingPassthroughChain.mem.get := TLFIFOFixer() := TLFragmenter(pbus.beatBytes, pbus.blockBytes) := _
      }
...
/** Mixin to add passthrough to rocket config
  */
class WithStreamingPassthrough
    extends Config((site, here, up) => { case StreamingPassthroughKey =>
      Some(StreamingPassthroughParams(depth = 8))
    })
