...
object FrequencyUtils {
  ...
  /** Adds up the squared error between the generated clocks (refClock / [integer] divider) and the requested
    * frequencies.
    *
    * @param refMHz
  ...
  /** Picks a candidate reference frequency by doing a brute-force search over multiples of the fastest requested clock.
    * Choose the smallest multiple that has an RMS error (across all output frequencies) that is: 1) zero or failing
    * that, 2) is within the relativeThreshold of the best or is less than the absoluteThreshold
    *
...
class SimplePllConfiguration(
  ...
  val referenceFreqMHz =
    FrequencyUtils.computeReferenceAsMultipleOfFastestClock(sinks.flatMap(_.take), maximumAllowableFreqMHz).freqMHz
