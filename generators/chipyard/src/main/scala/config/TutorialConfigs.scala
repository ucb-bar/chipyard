...
class TutorialSha3BlackBoxConfig
    extends Config(
    ...
    extends Config(
      new chipyard.iobinders.WithDontTouchIOBinders(false) ++
        ...
        new constellation.soc.WithGlobalNoC(
          constellation.soc.GlobalNoCParams(
            NoCParams(
              ...
              routingRelation =
                NonblockingVirtualSubnetworksRouting(TerminalRouterRouting(Mesh2DEscapeRouting()), 10, 1),
            )
          )
        ) ++
        ...
        new constellation.soc.WithSbusNoC(
          constellation.protocol.TLNoCParams(
            constellation.protocol.DiplomaticNetworkNodeMapping(
              inNodeMapping  = ListMap("Core 0" -> 0, "Core 1" -> 1, "serial-tl" -> 2),
              outNodeMapping =
                ListMap("system[0]" -> 3, "system[1]" -> 4, "system[2]" -> 5, "system[3]" -> 6, "pbus" -> 7),
            )
          ),
          true,
        ) ++
