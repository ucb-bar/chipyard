...
class WithBootROM
    extends Config((site, here, up) => {
      case BootROMLocated(x) => {
        ...
        val firesimBootROM  =
          new File(s"./target-rtl/chipyard/generators/testchipip/bootrom/bootrom.rv${site(XLen)}.img")
...
class WithFireSimHarnessClockBridgeInstantiator
    extends Config((site, here, up) => { case HarnessClockInstantiatorKey =>
      () => new FireSimClockBridgeInstantiator
    })

// Testing configurations
// This enables printfs used in testing
class WithScalaTestFeatures
    extends Config((site, here, up) => { case TracePortKey =>
      up(TracePortKey, site).map(_.copy(print = true))
    })
...
class WithMinimalFireSimDesignTweaks
    extends Config(
...
class WithMinimalFireSimHighPerfConfigTweaks
    extends Config(
...
/** Adds BlockDevice to WithMinimalFireSimHighPerfConfigTweaks
  */
class WithMinimalAndBlockDeviceFireSimHighPerfConfigTweaks
    extends Config(
      new WithFireSimHighPerfClocking ++
...
/** Adds Block device to WithMinimalFireSimHighPerfConfigTweaks
  */
class WithMinimalAndFASEDFireSimHighPerfConfigTweaks
    extends Config(
      new WithFireSimHighPerfClocking ++
...
class WithFireSimHighPerfConfigTweaks
    extends Config(
      new WithFireSimHighPerfClocking ++
        new WithFireSimDesignTweaks
    )
...
class WithFireSimTestChipConfigTweaks
    extends Config(
      ...
      new chipyard.config.WithTileFrequency(1000.0) ++                // Realistic tile frequency for a test chip
        ...
        new chipyard.config.WithMemoryBusFrequency(
          1000.0
        ) ++                                                          // Needs to be 1000 MHz to model DDR performance accurately
        new chipyard.config.WithPeripheryBusFrequency(500.0) ++       // Match the sbus and pbus frequency
        new chipyard.clocking.WithClockGroupsCombinedByName(
          ("uncore", Seq("sbus", "pbus", "fbus", "cbus", "implicit"), Seq("tile"))
        ) ++
...
/** ***************************************************************************** Full TARGET_CONFIG configurations.
  * These set parameters of the target being simulated.
  *
  * In general, if you're adding or removing features from any of these, you should CREATE A NEW ONE, WITH A NEW NAME.
  * This is because the manager will store this name as part of the tags for the AGFI, so that later you can reconstruct
  * what is in a particular AGFI. These tags are also used to determine which driver to build.
  */
...
class FireSimRocket1GiBDRAMConfig
    extends Config(
      new freechips.rocketchip.subsystem.WithExtMemSize((1 << 30) * 1L) ++
        new FireSimRocketConfig
    )

class FireSimRocketMMIOOnly1GiBDRAMConfig
    extends Config(
      new freechips.rocketchip.subsystem.WithExtMemSize((1 << 30) * 1L) ++
        new FireSimRocketMMIOOnlyConfig
    )

class FireSimRocket4GiBDRAMConfig
    extends Config(
      new freechips.rocketchip.subsystem.WithExtMemSize((1 << 30) * 4L) ++
        new FireSimRocketConfig
    )

class FireSimRocketMMIOOnly4GiBDRAMConfig
    extends Config(
      new freechips.rocketchip.subsystem.WithExtMemSize((1 << 30) * 4L) ++
        new FireSimRocketMMIOOnlyConfig
    )

class FireSimQuadRocketConfig
    extends Config(
      new WithDefaultFireSimBridges ++
        new WithDefaultMemModel ++
        new WithFireSimConfigTweaks ++
...
class FireSimSmallSystemConfig
    extends Config(
      new WithDefaultFireSimBridges ++
        new WithDefaultMemModel ++
        new WithBootROM ++
...
//********************************************************************
class FireSimLargeBoomAndRocketConfig
    extends Config(
      new WithDefaultFireSimBridges ++
        new WithDefaultMemModel ++
        new WithFireSimConfigTweaks ++
...
//******************************************************************
class FireSimGemminiRocketConfig
    extends Config(
      new WithDefaultFireSimBridges ++
        new WithDefaultMemModel ++
        new WithFireSimConfigTweaks ++
...
class FireSimLeanGemminiRocketConfig
    extends Config(
      new WithDefaultFireSimBridges ++
        new WithDefaultMemModel ++
        new WithFireSimConfigTweaks ++
...
//**********************************************************************************
class SupernodeFireSimRocketConfig
    extends Config(
      new WithFireSimHarnessClockBridgeInstantiator ++
...
//*********************************************************************************/
class FireSimMulticlockAXIOverSerialConfig
    extends Config(
...
//*********************************************************************************/
class FireSim16LargeBoomConfig
    extends Config(
      new WithDefaultFireSimBridges ++
        new WithDefaultMemModel ++
        new WithFireSimConfigTweaks ++
...
class FireSimRocketMMIOOnlyConfig
    extends Config(
...
class FireSimLeanGemminiRocketMMIOOnlyConfig
    extends Config(
      new WithDefaultMMIOOnlyFireSimBridges ++
        new WithDefaultMemModel ++
        new WithFireSimConfigTweaks ++
