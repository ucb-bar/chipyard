...
trait HarnessClockInstantiator {
  ...
  def requestClockHz(name: String, freqHzRequested: Double): Clock = {
    if (clockMap.contains(name)) {
      ...
      val clockBundle = Wire(Clock())
      clockMap(name) = (freqHzRequested, clockBundle)
      clockBundle
    }
  }
