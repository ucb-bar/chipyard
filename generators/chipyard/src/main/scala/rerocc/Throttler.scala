...
class TLThrottler(param_bitwidth: Int)(implicit p: Parameters) extends LazyModule {
  ...
  class Impl extends LazyModuleImp(this) {
    ...
    (node.in zip node.out) foreach { case ((in, edgeIn), (out, edgeOut)) =>
      ...
      when(req_counter > (max_req + offset) && epoch =/= 0.U && max_req =/= 0.U){
    ...
    when(epoch === 0.U && !io.req.fire){
      ...
      when (req_counter =/= 0.U){
        ...
        when(req_counter < max_req + offset){
          // when this epoch has less than allowed memory request, give more margin for next epoch
          offset := offset + (max_req  - req_counter)
        }
      }
      req_counter := 0.U
      when(temp_waiting) {
