...
import firrtl2.stage.{FirrtlSourceAnnotation, FirrtlStage, Forms, OutputFileAnnotation, RunFirrtlTransformAnnotation}
import firrtl2.{PrimOps, _}
import tapeout.macrolib.{PolarizedPort, PortPolarity, SRAMCompiler, SRAMGroup, SRAMMacro}
...
class MacroCompilerTransform extends Phase {//with DependencyAPIMigration {
  ...
  def execute(state: CircuitState): CircuitState = state.annotations.collect { case a: MacroCompilerAnnotation =>
    ...
    case Seq(anno: MacroCompilerAnnotation) =>
      ...
      val mems: Option[Seq[Macro]] = (memFileFormat match {
        ...
        case _            => tapeout.macrolib.Utils.readMDFFromPath(Some(memFile))
      }) match {
        case Some(x: Seq[tapeout.macrolib.Macro]) =>
          Some(filterForSRAM(Some(x)).getOrElse(List()).map { new Macro(_) })
        case _ => None
      }
      val libs: Option[Seq[Macro]] = tapeout.macrolib.Utils.readMDFFromPath(libFile) match {
        case Some(x: Seq[tapeout.macrolib.Macro]) =>
          Some(filterForSRAM(Some(x)).getOrElse(List()).map { new Macro(_) })
        case _ => None
      }
      val compilers: Option[tapeout.macrolib.SRAMCompiler] = tapeout.macrolib.Utils.readMDFFromPath(libFile) match {
        case Some(x: Seq[tapeout.macrolib.Macro]) =>
          if (useCompiler) {
...
class MacroCompilerOptimizations extends Phase {//SeqTransform with DependencyAPIMigration {
  override def prerequisites:          Seq[TransformDependency] = Forms.LowForm
  override def optionalPrerequisites:  Seq[TransformDependency] = Forms.LowFormOptimized
  override def optionalPrerequisiteOf: Seq[Dependency[Emitter]] = Forms.LowEmitters
  override def invalidates(a: Transform) = false
  ...
  def transforms: Seq[Transform] = Seq(
    ...
    new firrtl2.transforms.ConstantPropagation,
  ...
  def run(args: List[String]): Unit = {
    ...
    try {
      val macros = params.get(MacrosFormat) match {
        ...
        case _ =>
          filterForSRAM(tapeout.macrolib.Utils.readMDFFromPath(params.get(Macros))).get
