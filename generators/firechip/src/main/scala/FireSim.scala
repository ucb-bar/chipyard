...
/** Under FireSim's current multiclock implementation there can be only a single clock bridge. This requires, therefore,
  * that it be instantiated in the harness and reused across all supernode instances. This class attempts to memoize its
  * instantiation such that it can be referenced from within a ClockScheme function.
  */
