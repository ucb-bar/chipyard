...
// write some data to the flash using software (there is no hardware write
// controller)
void write_spiflash(uint8_t *data,
                    uint32_t len,
                    uint32_t addr,
                    uint8_t cmd,
                    uint8_t abytes,
...
int test_spiflash(uint32_t start, uint32_t size, uint8_t zero) {
  ...
  for (i = start; i < (start + size); i += 4) {
    ...
    if (!zero)
      check = 0xdeadbeef - i;
    if (data != check) {
      printf("Error reading address 0x%08x from SPI flash. Got 0x%08x, "
             "expected 0x%08x.\n",
             i,
             data,
             check);
...
int check_write(uint8_t *check, uint32_t len, uint32_t addr) {
  ...
  for (i = 0; i < len; i += 4) {
    ...
    if (check32 != data) {
      printf("Error reading address 0x%08x from SPI flash. Got 0x%02x, "
             "expected 0x%02x.\n",
             i + addr,
             data,
             check32);
