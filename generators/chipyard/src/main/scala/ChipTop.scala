...
/** The base class used for building chips. This constructor instantiates a module specified by the BuildSystem
  * parameter, named "system", which is an instance of DigitalTop by default. The diplomatic clocks of System, as well
  * as its implicit clock, is aggregated into the clockGroupNode. The parameterized functions controlled by
  * ClockingSchemeKey and GlobalResetSchemeKey drive clock and reset generation
  */

class ChipTop(implicit p: Parameters) extends LazyModule with BindingScope with HasIOBinders {
