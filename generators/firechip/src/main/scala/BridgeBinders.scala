...
import chipyard.iobinders.{ComposeIOBinder, GetSystemParameters, IOCellKey}
...
class WithFireSimIOCellModels
    extends Config((site, here, up) => { case IOCellKey =>
      FireSimIOCellParams()
    })

class WithTSIBridgeAndHarnessRAMOverSerialTL
    extends OverrideHarnessBinder({ (system: CanHavePeripheryTLSerial, th: FireSim, ports: Seq[ClockedIO[SerialIO]]) =>
      {
        ports.map { port =>
          ...
          TSIBridge(
            ...
            ram.module.io.tsi,
            p(ExtMem).map(_ => MainMemoryConsts.globalName),
...
class WithNICBridge
    extends OverrideHarnessBinder({ (system: CanHavePeripheryIceNIC, th: FireSim, ports: Seq[ClockedIO[NICIOvonly]]) =>
      {
...
class WithUARTBridge
    extends OverrideHarnessBinder({ (system: HasPeripheryUARTModuleImp, th: FireSim, ports: Seq[UARTPortIO]) =>
...
class WithAXIOverSerialTLCombinedBridges
    extends OverrideHarnessBinder({ (system: CanHavePeripheryTLSerial, th: FireSim, ports: Seq[ClockedIO[SerialIO]]) =>
      {
        implicit val p = GetSystemParameters(system)
        ...
        p(SerialTLKey).map({ sVal =>
          ...
          ports.map({ port =>
            ...
            TSIBridge(
              th.harnessBinderClock,
              harnessMultiClockAXIRAM.module.io.tsi,
            ...
            (harnessMultiClockAXIRAM.mem_axi4.get.zip(harnessMultiClockAXIRAM.memNode.get.edges.in)).map {
              case (axi4, edge) =>
                val nastiKey = NastiParameters(
                  axi4.bits.r.bits.data.getWidth,
...
class WithTracerVBridge
    extends ComposeHarnessBinder({ (system: CanHaveTraceIOModuleImp, th: FireSim, ports: Seq[TraceOutputTop]) =>
      {
...
class WithDromajoBridge
    extends ComposeHarnessBinder({ (system: CanHaveTraceIOModuleImp, th: FireSim, ports: Seq[TraceOutputTop]) =>
      ports.map { p => p.traces.map(tileTrace => DromajoBridge(tileTrace)(system.p)) }; Nil
    })

class WithTraceGenBridge
    extends OverrideHarnessBinder({ (system: TraceGenSystemModuleImp, th: FireSim, ports: Seq[Bool]) =>
      ports.map { p => GroundTestBridge(th.harnessBinderClock, p)(system.p) }; Nil
    })

class WithFireSimMultiCycleRegfile
    extends ComposeIOBinder({ (system: HasTilesModuleImp) =>
      {
...
class WithDefaultFireSimBridges
    extends Config(
...
class WithDefaultMMIOOnlyFireSimBridges
    extends Config(
      new WithTSIBridgeAndHarnessRAMOverSerialTL ++
        new WithUARTBridge ++
        new WithBlockDeviceBridge ++
