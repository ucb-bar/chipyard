...
parser.add_argument(
    ...
    type=str,
    required=True,
    help="Path to hierarchy JSON emitted by firtool. Must include DUT as a module.",
)
parser.add_argument(
    "--top-hier-json",
    type=str,
...
parser.add_argument(
    ...
    type=str,
    required=True,
    help="Path to input filelist that has all modules (relative paths).",
)
parser.add_argument("--dut", type=str, required=True, help="Name of the DUT module.")
parser.add_argument(
    "--model", type=str, required=True, help="Name of the Model module."
)
parser.add_argument(
    "--out-dut-filelist",
    type=str,
    required=True,
    help="Path to output filelist including all modules under the DUT.",
)
parser.add_argument(
    "--out-model-filelist",
    type=str,
    required=True,
    help="Path to output filelist including all modules under the MODEL.",
)
parser.add_argument(
    "--out-model-hier-json",
    type=str,
    required=True,
    help="Path to updated hierarchy JSON emitted by this script.",
)
parser.add_argument(
    "--target-dir",
    type=str,
    required=True,
    help="Path to where module sources are located (combined with --in-all-filelist gives the absolute path to module sources).",
)
...
def get_modules_in_filelist(verilog_module_filename, cc_filelist):
    with open(args.in_all_filelist) as fl:
        ...
        for line in lines:
            ...
            if (ext == "v") or (ext == "sv"):
                modules = get_modules_in_verilog_file(
                    os.path.join(args.gcpath, basepath)
                )
...
def get_modules_under_hier(hier, child_to_ignore=None):
    with open(hier) as hj:
        ...
        modules_under_hier = set(
            bfs_collect_modules(hj_data, child_to_ignore=child_to_ignore)
        )
    ...
    while len(q) != 0:
            ...
            try:
                ...
                if parent is not None and ((parent, mod) not in updated_submodule):
                    parent_file = os.path.join(
                        args.gcpath, verilog_module_filename[parent]
                    )
        ...
        for c in child:
            if c["module_name"] != args.dut:
                q.append(
                    (c["instance_name"], c["module_name"], c["instances"], new_mod)
                )
...
def uniquify_modules_under_model(
    modules_under_model, common_modules, verilog_module_filename
):
...
def main():
    ...
    # write top filelist
    write_verilog_filelist(
        modules_under_top, verilog_module_filename, args.out_dut_filelist
    )

    # rename modules that are common
    uniquify_modules_under_model(
        modules_under_model, common_modules, verilog_module_filename
    )
    uniquified_modules_under_model = get_modules_under_hier(
        args.out_model_hier_json, args.dut
    )

    # write model filelist
    write_verilog_filelist(
        uniquified_modules_under_model, verilog_module_filename, args.out_model_filelist
    )
