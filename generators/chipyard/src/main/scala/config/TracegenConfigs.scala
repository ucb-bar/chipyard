...
class BarCacheTraceGenConfig extends Config(
  new tracegen.WithTraceGen()(List.fill(2) { DCacheParams(nMSHRs = 8, nSets = 16, nWays = 2) }) ++
  new barcache.WithBarCache((0 until 2)) ++
  new AbstractTraceGenConfig)
...
class BoomTraceGenL2Config extends Config(
  new tracegen.WithBoomTraceGen()(List.fill(2) { DCacheParams(nMSHRs = 8, nSets = 16, nWays = 2) }) ++
  new freechips.rocketchip.subsystem.WithInclusiveCache ++
  new AbstractTraceGenConfig)

class BarCacheTraceGenL2Config extends Config(
  new tracegen.WithTraceGen()(List.fill(2) { DCacheParams(nMSHRs = 8, nSets = 16, nWays = 2) }) ++
  new barcache.WithBarCache((0 until 2)) ++
  new freechips.rocketchip.subsystem.WithInclusiveCache ++
  new AbstractTraceGenConfig)
