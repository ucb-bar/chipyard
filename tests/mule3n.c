...
#define MULE3N_FUNCT3 0x0
#define MULE3N_FUNCT7 0x0e

struct mule3n_test_case {
  int64_t a;
  int64_t b;
};

static inline uint64_t mule3n_hw(int64_t a, int64_t b)
{
  uint64_t rd;
  ...
  asm volatile (
      ...
      : "=r" (rd)
      : "r" ((uint64_t)a), "i" (MULE3N_FUNCT7), "i" (MULE3N_FUNCT3), "r" ((uint64_t)b));
...
static inline uint64_t mule3n_ref(int64_t a, int64_t b)
{
...
int main(void)
{
  static const struct mule3n_test_case tests[] = {
      {0, 0},
      {1, 1},
      {7, 9},
      {-7, 9},
  ...
  for (unsigned i = 0; i < (sizeof(tests) / sizeof(tests[0])); i++) {
    ...
    const uint64_t mule3n = mule3n_hw(a, b);
    const uint64_t mul    = mul_hw(a, b);
    const uint64_t ref    = mule3n_ref(a, b);

    if (mule3n != ref || mul != ref || mule3n != mul) {
      printf("MULE3N mismatch at test %u\n", i);
      printf("  a      = 0x%016" PRIx64 " (%" PRId64 ")\n", (uint64_t)a, a);
      printf("  b      = 0x%016" PRIx64 " (%" PRId64 ")\n", (uint64_t)b, b);
      printf("  mule3n = 0x%016" PRIx64 "\n", mule3n);
