    ...
    extends Config(
      new chipyard.iobinders.WithDontTouchIOBinders(false) ++ // TODO: hack around dontTouch not working in SFC
        new fftgenerator.WithFFTGenerator(
          ...
          width     = 16,
          decPt     = 8,
        ) ++                                                  // add 8-point mmio fft at the default addr (0x2400) with 16bit fixed-point numbers.
...
class GCDAXI4BlackBoxRocketConfig
    extends Config(
      new chipyard.example.WithGCD(
        useAXI4     = true,
        useBlackBox = true,
      ) ++ // Use GCD blackboxed verilog, connect by AXI4->Tilelink
        new freechips.rocketchip.subsystem.WithNBigCores(1) ++
        new chipyard.config.AbstractConfig
    )
...
class StreamingPassthroughRocketConfig
    extends Config(
...
class StreamingFIRRocketConfig
    extends Config(
...
class ManyMMIOAcceleratorRocketConfig
    extends Config(
      new chipyard.iobinders.WithDontTouchIOBinders(false) ++ // TODO: hack around dontTouch not working in SFC
        new fftgenerator.WithFFTGenerator(
          numPoints = 8,
