...
import tapeout.macrolib.{Input => _, Output => _, _}
...
class Macro(srcMacro: SRAMMacro) {
  ...
  val firrtl_ports: Seq[FirrtlMacroPort] = srcMacro.ports.map { new FirrtlMacroPort(_) }

  val writers:     Seq[FirrtlMacroPort] = firrtl_ports.filter(p => p.isWriter)
  val readers:     Seq[FirrtlMacroPort] = firrtl_ports.filter(p => p.isReader)
  val readwriters: Seq[FirrtlMacroPort] = firrtl_ports.filter(p => p.isReadWriter)
  ...
  val tpe: BundleType = BundleType(firrtl_ports.flatMap(_.tpe.fields))

  private val modPorts = firrtl_ports.flatMap(_.ports) ++
...
object Utils {
  def filterForSRAM(s: Option[Seq[tapeout.macrolib.Macro]]): Option[Seq[tapeout.macrolib.SRAMMacro]] = {
    s match {
      case Some(l: Seq[tapeout.macrolib.Macro]) =>
        Some(l.filter { _.isInstanceOf[tapeout.macrolib.SRAMMacro] }.map { m => m.asInstanceOf[tapeout.macrolib.SRAMMacro] })
      case _ => None
    }
  }
  // This utility reads a conf in and returns MDF like tapeout.macrolib.Utils.readMDFFromPath
  def readConfFromPath(path: Option[String]): Option[Seq[tapeout.macrolib.Macro]] = {
    path.map(p => Utils.readConfFromString(FileUtils.getText(p)))
  }
  def readConfFromString(str: String): Seq[tapeout.macrolib.Macro] = {
  ...
  def findSRAMCompiler(s: Option[Seq[tapeout.macrolib.Macro]]): Option[tapeout.macrolib.SRAMCompiler] = {
    s match {
      case Some(l: Seq[tapeout.macrolib.Macro]) =>
        l.collectFirst { case x: tapeout.macrolib.SRAMCompiler =>
          x
        }
      case _ => None
    }
  }
  def buildSRAMMacros(s: tapeout.macrolib.SRAMCompiler): Seq[tapeout.macrolib.SRAMMacro] = {
    for {
      g <- s.groups
      d <- g.depth
      w <- g.width
      vt <- g.vt
    } yield tapeout.macrolib.SRAMMacro(
      makeName(g, d, w, vt),
      w,
      d,
      g.family,
  ...
  def buildSRAMMacro(g: tapeout.macrolib.SRAMGroup, d: Int, w: Int, vt: String): tapeout.macrolib.SRAMMacro = {
    tapeout.macrolib.SRAMMacro(
      makeName(g, d, w, vt),
      w,
      d,
      g.family,
  ...
  def makeName(g: tapeout.macrolib.SRAMGroup, depth: Int, width: Int, vt: String): String = {
